...
class Square(MaskShape):
    ...
    def setDirectly(self, parameters):
        ...
        if not self.parameters['position'] == parameters[1] or not self.parameters['width'] == parameters[2] or not self.parameters['height'] == parameters[3]:

            self.parameters['position'] = list(parameters[1])
            self.parameters['width']    = float(parameters[2])
